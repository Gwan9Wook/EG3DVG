...
class EG(nn.Module):
    ...
    def __init__(self, num_class=256, num_obj_class=485,
        ...
        self.decoder_pos_embed = PositionEmbeddingLearned(3, d_model)
        ...
        for i in range(self.num_decoder_layers):
            self.decoder.append(BiDecoderLayer(
                ...
                super_refine=True if i % 3 == 2 else False
            ))
    ...
    def forward(self, inputs):
        ...
        for bs in range(source_xzy.shape[0]):
            super_xyz = scatter_mean(source_xzy[bs], superpoint[bs], dim=0).unsqueeze(
                0)  # [1, super_num, 3]  计算每个超点的平均坐标，即得到中心坐标
        ...
        # STEP 6.Proposals
        proposal_center, proposal_size, proposal_mask_feat = self.proposal_head(
            cluster_feature,
        ...
        attn_mask_list = []
        pred_mask_list = []
        for bs in range(proposal_mask_feat.shape[0]):
            bs_query = proposal_mask_feat[bs].unsqueeze(0)
            _, pred_masks, attn_masks = self.prediction_head(
                bs_query,
                super_features[bs].unsqueeze(0).transpose(1, 2)
            )
            attn_mask_list.append(attn_masks)
            pred_mask_list.append(pred_masks.squeeze(0))
        end_points['proposal_pred_masks'] = pred_mask_list
        ...
        for i in range(self.num_decoder_layers):
            ...
            query, super_features = self.decoder[i](
                ...
                xyz_embed=self.decoder_pos_embed(points_xyz),
                points_xyz=points_xyz,
                super_features=super_features,
                attn_mask_list=attn_mask_list
                # global_text_feats=(gamma[:, i, :], beta[:, i, :])
            )  # (B, V, F)
            # step project
            if self.contrastive_align_loss:
            ...
            base_xyz, base_size, mask_feat = self.prediction_heads[i](
            ...
            attn_mask_list = []
            pred_mask_list = []
            for bs in range(mask_feat.shape[0]):
                bs_query = mask_feat[bs].unsqueeze(0)
                _, pred_masks, attn_masks = self.prediction_head(
                    bs_query,
                    super_features[bs].unsqueeze(0).transpose(1, 2)
                )
                attn_mask_list.append(attn_masks)
                pred_mask_list.append(pred_masks.squeeze(0))
            end_points[f'{prefix}pred_masks'] = pred_mask_list
