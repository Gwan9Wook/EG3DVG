...
class ClsAgnosticPredictHead(nn.Module):
    def __init__(self, num_class, num_heading_bin, num_proposal,
        ...
        self.x_query = nn.Sequential(
            nn.Conv1d(seed_feat_dim, seed_feat_dim * 2, 1),
            nn.ReLU(),
            nn.Conv1d(seed_feat_dim * 2, seed_feat_dim * 2, 1),
            nn.ReLU(),
            nn.Conv1d(seed_feat_dim * 2, seed_feat_dim, 1)
        )
    ...
    def forward(self, features, base_xyz, end_points, prefix=''):
        ...
        return center, pred_size, self.x_query(features).transpose(1, 2)
