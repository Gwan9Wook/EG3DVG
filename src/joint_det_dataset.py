...
class Joint3DDataset(Dataset):
    ...
    def __init__(self, dataset_dict={'sr3d': 1, 'scannet': 10},
        ...
        if self.split != 'train':
            ...
            file_name = f'{self.data_path}your_text_save_dir/your_text_data.pkl'
            # file_name = 'sr3d_val_preprocessed_data.pkl'
            # with open(file_name, 'wb') as file:
            #     pickle.dump(self.annos, file)
    ...
    def _get_target_boxes(self, anno, scan):
        ...
        one_hot_dict = getattr(DC, f'one_hot_{dataset}')
        point_class_label = np.ones(len(scan.pc)) * 242
        for objects in scan.three_d_objects:
            if objects['instance_label'] in one_hot_dict:
                point_class_label[objects['points']] = one_hot_dict[objects['instance_label']]

        return bboxes, box_label_mask, point_instance_label, gt_masks, point_class_label
    ...
    def __getitem__(self, index):
        ...
        gt_bboxes, box_label_mask, point_instance_label, gt_masks, point_class_label = \
        ...
        ##########################
        # STEP Sampling Negative #
        ##########################
        is_negative = False
        if anno['dataset'] != 'scannet':
            annotations = [item for item in self.annos if
                           anno['scan_id'] == item['scan_id'] and item['dataset'] != 'scannet']

            obj_id = anno['target_id']

            negative_sample = [item for item in annotations if
                               item['target'] == anno['target'] and item['target_id'] != obj_id]

            if len(negative_sample) != 0:
                negative_selected = random.choice(negative_sample)
                negative_tokens_positive, negative_positive_map, negative_modify_positive_map, negative_pron_positive_map, \
                    negative_other_entity_map, negative_auxi_entity_positive_map, negative_rel_positive_map = self._get_token_positive_map_by_parse(
                    negative_selected, auxi_box=None)
                negative_utterances = ' '.join(
                    negative_selected['utterance'].replace(',', ' ,').split()) + ' . not mentioned'
                is_negative = True
        if not is_negative:
            negative_positive_map = np.zeros((MAX_NUM_OBJ, 256))
            negative_modify_positive_map = np.zeros((MAX_NUM_OBJ, 256))
            negative_pron_positive_map = np.zeros((MAX_NUM_OBJ, 256))
            negative_other_entity_map = np.zeros((MAX_NUM_OBJ, 256))
            negative_auxi_entity_positive_map = np.zeros((MAX_NUM_OBJ, 256))
            negative_rel_positive_map = np.zeros((MAX_NUM_OBJ, 256))
            negative_utterances = "none"
        ...
        ret_dict.update({
            ...
            "negative_positive_map": negative_positive_map.astype(np.float32),
            "negative_modify_positive_map": negative_modify_positive_map.astype(np.float32),
            "negative_pron_positive_map": negative_pron_positive_map.astype(np.float32),
            "negative_other_entity_map": negative_other_entity_map.astype(np.float32),
            "negative_auxi_entity_positive_map": negative_auxi_entity_positive_map.astype(np.float32),
            "negative_rel_positive_map": negative_rel_positive_map.astype(np.float32),
            "negative_utterances": negative_utterances,
        })
