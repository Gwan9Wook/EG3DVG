...
class GroundingEvaluator:
    ...
    def evaluate_bbox_by_pos_align(self, end_points, prefix):
        ...
        for bs in range(len(end_points['last_pred_masks'])):
            ...
            pred_masks_ = torch.gather(pred_masks_, 2,
                                       superpoints.unsqueeze(1).expand(-1, 256, -1))  # (1, 256, 50000)
        ...
        pred_bbox2 = self.get_boxes_from_masks(pred_masks, end_points['point_clouds'][..., :3])
        pred_bbox = (pred_bbox + pred_bbox2) / 2
    ...
    def get_boxes_from_masks(self, masks, points):
        """
        Args:
            masks: [B, 256, 50000] binary tensor
            points: [B, 50000, 3] float tensor

        Returns:
            boxes: [B, 256, 6] (center_x, center_y, center_z, dx, dy, dz)
        """
        B, Q, N = masks.shape
        device = masks.device

        # Expand points to [B, 1, N, 3] → broadcast to [B, Q, N, 3]
        points_exp = points.unsqueeze(1).expand(-1, Q, -1, -1)  # [B, Q, N, 3]

        # Expand masks to match point dims: [B, Q, N, 1] → [B, Q, N, 3]
        masks_exp = masks.unsqueeze(-1).expand(-1, -1, -1, 3).bool()  # [B, Q, N, 3]

        # Replace non-masked points with large positive/negative values
        masked_min = points_exp.clone()
        masked_min[~masks_exp] = 1e10  # for min
        min_xyz = masked_min.min(dim=2).values  # [B, Q, 3]

        masked_max = points_exp.clone()
        masked_max[~masks_exp] = -1e10  # for max
        max_xyz = masked_max.max(dim=2).values  # [B, Q, 3]

        center = (min_xyz + max_xyz) / 2  # [B, Q, 3]
        size = (max_xyz - min_xyz)  # [B, Q, 3]
